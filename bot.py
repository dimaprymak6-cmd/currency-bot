...
def parse_listings(html: str) -> list:
    ...
    # Метод 1: стандартные li карточки 999.md
    items = soup.select("li.ads-list-photo-item")
    if items:
        logger.info(f"Метод 1: нашли {len(items)} карточек")
        for item in items:
            link = (
                item.select_one("a.ads-list-photo-item-title") or
                item.select_one("a[href*='/ru/view/']")
            )
            if not link:
                continue
            href = link.get("href", "")
            ...
            if not m:
                continue
            price_tag = (
                item.select_one(".ads-list-photo-item-price-wrapper") or
    ...
    # Метод 2: ищем все ссылки напрямую
    logger.info("Метод 2: ищем ссылки /ru/view/")
    seen = set()
    ...
    logger.info(f"Метод 2: нашли {len(listings)} объявлений")
    return listings


def fetch_listings():
    """Использует cloudscraper для обхода антибот защиты 999.md."""
    all_listings = {}
    network_error = True
    ...
    for url in SEARCH_URLS:
        try:
            ...
            if resp.status_code == 200:
                ...
                for item in parse_listings(resp.text):
...
def main_keyboard(mon: bool) -> InlineKeyboardMarkup:
    btn = (
        ...
        if mon else
        InlineKeyboardButton("▶️ Запустить мониторинг", callback_data="start")
    )
...
async def send_check_report(bot: Bot, new_count: int, total: int, error: bool = False):
    ...
    if error:
        text = (
            f"⚠️ `{now}` — Не удалось подключиться к 999\\.md\\.\n"
        ...
        text = (
            f"🔄 `{now}` — Проверка ок\\. "
            f"Объявлений с яблоками: {escape_md(str(total))}\\. "
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "start":
        ...
        await query.edit_message_text(
            ...
            f"📋 После каждой проверки — тихое сообщение\\.\n"
        ...
        if listings is None:
            ...
            if new_listings:
                ...
                result = (
                    ...
                    f"Всего объявлений с яблоками: *{len(listings)}* шт\\."
                )
        ...
        await query.edit_message_text(
            f"🗑 *Кэш сброшен\\!*\n\n"
            f"Удалено: *{old_count}* записей\\.\n"
            f"При следующей проверке все объявления покажутся как новые\\.",
        ...
        text = (
            ...
            "⏱ *Изменить интервал:*\n"
            "Railway → Variables → CHECK\\_INTERVAL\n"
            "60 \\= каждую минуту, 300 \\= каждые 5 мин"
        )
        await query.edit_message_text(text, parse_mode="MarkdownV2", reply_markup=back_keyboard())
